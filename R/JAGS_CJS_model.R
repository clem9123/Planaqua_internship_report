...
model <- function(){
  ...
  for (t in 3:(noccas-1)){
    for (tr in 1:4){ # experiment time 2
  ...
  for (t in 1:(noccas-1)){
    for (tr in 1:4) {
      gamma[1,1,tr,t] <- phi1_aux[tr,t] * (1-psi12_aux[tr,t])   # Pr(alive and small t -> alive and small t+1)    survivalin same size class
      gamma[1,2,tr,t] <- phi1_aux[tr,t] * psi12_aux[tr,t]       # Pr(alive and small t -> alive and medium t+1)   survival and growth
      gamma[1,3,tr,t] <- 0                                      # Pr(alive and small t -> alive and large t+1)    impossible to shrink
      gamma[1,4,tr,t] <- (1-phi1_aux[tr,t])                     # Pr(alive and small t -> dead t+1)               mortality
      gamma[2,1,tr,t] <- 0                                      # Pr(alive and medium t -> alive and small t+1)   impossible to shrink
      gamma[2,2,tr,t] <- phi2_aux[tr,t] * (1-psi23_aux[tr,t])   # Pr(alive and medium t -> alive and medium t+1)  survival in same size class
      gamma[2,3,tr,t] <- phi2_aux[tr,t] * psi23_aux[tr,t]       # Pr(alive and medium t -> alive and large t+1)   survival and growth
      gamma[2,4,tr,t] <- (1-phi2_aux[tr,t])                     # Pr(alive and medium t -> dead t+1)              mortality
      gamma[3,1,tr,t] <- 0                                      # Pr(alive and large t -> alive and small t+1)    impossible to shrink
      gamma[3,2,tr,t] <- 0                                      # Pr(alive and large t -> alive and medium t+1)   impossible to shrink
      gamma[3,3,tr,t] <- phi3_aux[tr,t]                         # Pr(alive and large t -> alive and large t+1)    survivalin same size class
      gamma[3,4,tr,t] <- (1-phi3_aux[tr,t])                     # Pr(alive and large t -> dead t+1)               mortality
      gamma[4,1,tr,t] <- 0                                      # Pr(dead t -> alive and small t+1)               no resurection
      gamma[4,2,tr,t] <- 0                                      # Pr(dead t -> alive and medium t+1)              no resurection
      gamma[4,3,tr,t] <- 0                                      # Pr(dead t -> alive and large t+1)               no resurection
      gamma[4,4,tr,t] <- 1                                      # Pr(dead t -> dead t+1)                          dead stay s=dead (certain)
    }
  }
